...
CREATE TABLE IF NOT EXISTS files (id INTEGER PRIMARY KEY, snippet_id INTEGER NOT NULL, user_id INTEGER NOT NULL, name TEXT NOT NULL, content BLOB NOT NULL, created_at TEXT NOT NULL, updated_at TEXT NOT NULL, FOREIGN KEY(snippet_id) REFERENCES snippets(id) ON UPDATE CASCADE ON DELETE CASCADE, FOREIGN KEY(user_id) REFERENCES users(id) ON UPDATE CASCADE ON DELETE CASCADE);
