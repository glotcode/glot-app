...
import glot_backend/spam_classifier/adapter/http/client as spam_classifier_client
import glot_backend/spam_classifier/adapter/postgres/storage
...
pub fn new(http_pools: Pools, db: database.Db) -> system_ports.SystemPorts {
  system_ports.SystemPorts(
    ...
    spam_classifier: ports.Ports(
      spam_classifier_client.new(http_pools.spam_classifier),
      storage.new(db),
    ),
  )
}
